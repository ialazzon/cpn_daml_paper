...
//		unit.setColorSetType("UNIT");
//		unit.setWith("unit");
//		globbox.getDeclarations().add(unit);
	...
	public static void transform_template(Page page, Template t ) {
		Trans trans = CpntoolsFactory.eINSTANCE.createTrans();
		trans.setText(t.getName());
		page.getTranss().add(trans);
		for(Party p:t.getSignatory())
			transform_signatory(page, trans, p);
		
		for(Choice c:t.getChoice()) {
			Place p = transform_controller(page, trans, c.getController().get(0), c.getName());
			linkTemplateWithController(page,trans,c.getController().get(0),p);
		}
		
	}
	
	public static void transform_signatory(Page page, Trans trans, Party signatory) {
	...
	public static Place transform_controller(Page page, Trans trans, Party controller, String choiceName) {
		Cpnet net = page.getBinder().getCpnet();
